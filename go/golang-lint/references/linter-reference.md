...
| Testing | thelper, paralleltest, usetesting; testifylint when Testify is used | Test hygiene and best practices |
...
- **testifylint** — Optional; enforces Testify best practices when the project uses Testify (e.g., `assert.Equal(t, expected, actual)` over `assert.True(t, expected == actual)`)
