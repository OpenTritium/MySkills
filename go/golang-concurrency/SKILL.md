...
9. **Track goroutine leaks in tests** with goleak when the project already uses it; otherwise assert explicit shutdown and run `go test -race`
...
For pipeline patterns (fan-out/fan-in, bounded workers, generator chains, and Go 1.23+ iterators), see [Pipelines and Worker Pools](references/pipelines.md).
...
- tests: `go.uber.org/goleak` when already present; otherwise explicit shutdown assertions and `go test -race`
