...
See `golang-error-handling` for the single handling rule. See `golang-troubleshooting` for using signals to diagnose production issues. See `golang-security` for protecting pprof endpoints and avoiding PII in logs. See `golang-context` for propagating trace context across service boundaries. Use the project's Prometheus UI or existing query tooling for PromQL exploration.
...
- **[Alerting](references/alerting.md)** — Proactive problem detection. Covers the four golden signals, reusable Go runtime alerts, severity levels, and common mistakes that break alerting (using `irate` instead of `rate`, missing `for:` duration to avoid flapping).
...
2. During migration, use the project's existing adapter or a small local `slog.Handler` bridge only if changing the output backend in one step is not practical.
...
- [ ] **Dashboards and alerts exist** — the PromQL from your metric comments is wired into Grafana dashboards and Prometheus alerting rules, with thresholds reviewed against the service's SLOs.
