...
When adding a database, cache, broker, or proxy, define alerts from the dependency's exported metrics and the service's SLOs:

1. Confirm the exporter and metric names in the deployed version.
2. Start with availability, latency, errors, and saturation.
3. Set thresholds and `for:` durations from observed baseline and recovery behavior.
4. Keep rules in version control and review them with the dashboard and runbook.
