...
When the standard library does not cover the use case, favor well-known packages from the Go team (`golang.org/x/...`) or established organizations over obscure alternatives, and verify maintenance, license, and vulnerability history before adoption.
...
Before pinning a version, inspect the module's available versions, importers, and known vulnerabilities on pkg.go.dev → See pkg.go.dev.
