...
**Migration guidance**: For existing projects heavily invested in third-party loggers, migration is optional. For new projects, prefer `slog`. Use a project-approved `slog.Handler` adapter only when an existing backend must be retained during migration. Go 1.24 added `slog.DiscardHandler` for silent loggers.
