---
name: golang-testing
description: "Production-ready Go tests: table-driven and subtests, parallel tests, fuzzing, fixtures, optional goroutine leak detection, coverage, integration tests, and idiomatic test naming. Use when writing or reviewing tests, choosing a testing approach, setting up Go test CI, or debugging flaky or slow tests. Use `golang-benchmark` for measurement methodology."
license: MIT
---
...
6. Packages with goroutines SHOULD use `goleak.VerifyTestMain` when the project already depends on goleak; otherwise use `-race`, explicit shutdown assertions, and bounded tests
7. Prefer the standard `testing` package; keep Testify only when the project already depends on it
...
## Optional Goroutine Leak Detection

When the project already depends on `go.uber.org/goleak`, use it to detect leaking goroutines, especially for concurrent code. Do not add it to every package by default:
...
Many test best practices are enforced automatically by linters: `thelper` and `paralleltest`. Use `testifylint` only when the project already uses Testify. See the `golang-lint` skill for configuration and usage.
