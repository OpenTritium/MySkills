---
name: golang-gopls
description: "Golang semantic code intelligence via `gopls`, the official Go language server — go-to-definition, find references, call/implementation hierarchy, workspace symbol search, package API discovery, diagnostics, safe rename, refactors (extract/inline/fill/rewrite code actions), formatting, and generated tests. Reaches an agent via gopls's own MCP server (`go_*` tools), a host-provided native LSP tool, or the `gopls` CLI. Use when navigating or refactoring Go code — jumping to a definition, finding call sites before a rename, understanding a file's or package's dependencies, running diagnostics after an edit, or extracting/inlining/renaming. Not for the published ecosystem — packages not in your `go.mod`, versions, licenses, importers — → See pkg.go.dev. Not for a whole-tree vulnerability audit → See `golang-security` skill (`govulncheck`)."
license: MIT
---


**Dependencies:** `gopls` — `go install golang.org/x/tools/gopls@latest` (v0.20+). A host-provided native `LSP` tool may need separate configuration; see [references/mcp.md](references/mcp.md).

`gopls` is the official Go language server. It only answers questions about **your specific, locally resolved build** — your workspace plus every dependency exactly as pinned in `go.sum`, including `replace` directives. For a package that isn't part of that build (versions, docs, licenses, CVEs of something you haven't added yet), → See pkg.go.dev instead.

## Three ways to reach gopls

Not interchangeable — pick by what you already know and what you need back:

- **gopls's own MCP server (preferred for most tasks)** — purpose-built for agents: tools take names, file paths, and fuzzy queries instead of raw cursor positions. Register `gopls mcp` in the host's MCP configuration. It runs headless over stdio, has no editor attached, and only sees files saved to disk — the right default for an agent-only workflow. See [references/mcp.md](references/mcp.md) for every tool.
- **A native `LSP` tool** — a host-provided editor-style integration. Configure it with `gopls` when available. Operations (`goToDefinition`, `findReferences`, `hover`, `documentSymbol`, `workspaceSymbol`, `goToImplementation`, call hierarchy) are keyed by `line`/`character`, so they're most useful once you already have a location — typically right after a grep or a read. Some hosts also push compiler diagnostics into context automatically after edits.
...
- For anything not tied to that build (version history, license, ecosystem-wide importers, CVEs of a package not yet added) → See pkg.go.dev — it queries pkg.go.dev directly, no local checkout needed.
...
Use the local Go skill index and the tool boundaries in this file to choose between gopls, pkg.go.dev, Context7, and govulncheck.
