...
- **godoc** — your doc comments render on [pkg.go.dev](https://pkg.go.dev). Use `go doc` locally to preview; to inspect how a published package renders its docs, symbols, and examples, → See pkg.go.dev.
...
| ------------ | ----------- | -------------------------------------------- |
| REST/HTTP    | OpenAPI 3.x | ogen or another repository-approved OpenAPI generator |
| Event-driven | AsyncAPI    | Manual or code-gen                           |
| gRPC         | Protobuf    | buf, grpc-gateway                            |

Prefer a reviewed OpenAPI contract and reproducible generation when possible. See [Application Documentation](./references/application.md#api-documentation) for details.
