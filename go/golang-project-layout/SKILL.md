...
After settling on the architecture, **ask the developer** whether manual constructor injection is sufficient or whether `github.com/samber/do/v2` is justified by graph size, scopes, or lifecycle management. See the `golang-dependency-injection` skill for the decision boundary and wiring rules.
...
For application configuration boundaries and typed config loading, see [config reference](references/config.md).
...
→ See `golang-cli` skill for CLI boundaries and command layout. → See `golang-dependency-injection` skill for manual or do/v2 wiring. → See `golang-lint` skill for golangci-lint configuration. → See `golang-continuous-integration` skill for CI/CD pipeline setup. → See `golang-design-patterns` skill for architectural patterns. → See `golang-refactoring` skill for safely moving or splitting existing code into the layout above via type-alias gradual code repair and staged PRs, without a big-bang break.
