...
→ See `golang-concurrency` skill for `singleflight` API details and `sync.Map` vs `RWMutex` decision guidance. Use `golang.org/x/sync/singleflight` by default; add a typed wrapper only when it improves the local API and benchmark results justify it.
