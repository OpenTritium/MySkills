...
**Diagnose first:** use `go tool pprof` to confirm JSON encode/decode is material to CPU or allocation cost, then compare representative payloads with `go test -bench=. -benchmem` and `benchstat`.

Keep `encoding/json` as the default. For a measured hot path, consider in this order:

1. Reduce payload size and avoid unnecessary marshal/unmarshal cycles.
2. Stream large bodies with `json.Decoder`/`json.Encoder` instead of buffering them.
3. Add targeted `MarshalJSON`/`UnmarshalJSON` methods or code generation for stable hot-path types.
4. Benchmark a drop-in alternative such as `github.com/bytedance/sonic`, `github.com/goccy/go-json`, or `github.com/json-iterator/go` against the project's payloads and supported platforms.

Third-party JSON libraries are optional optimizations, not default replacements. Check compatibility, semantics, architecture support, security posture, and maintenance before adopting one. Experimental standard-library JSON modes should be enabled only by an explicit project decision.
