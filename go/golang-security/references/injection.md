...
### Keep the SQL abstraction explicit

Use `database/sql` with parameterized placeholders for portable code, or native `pgx/v5` for PostgreSQL-specific features. A helper or query library does not make interpolated SQL safe; validate identifiers separately and pass values as driver arguments.
