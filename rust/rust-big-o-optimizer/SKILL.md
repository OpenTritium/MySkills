---
name: rust-big-o-optimizer
description: 'Review Rust algorithm complexity and data-structure choices. Use for nested loops, redundant traversal, missing memoization, over-sorting, iterator misuse, top-K selection, or HashMap/BTreeMap/BinaryHeap tradeoffs. Keywords: Big-O, time complexity, space complexity, O(N), O(N²), O(log N), algorithm, data structure, HashMap, HashSet, BTreeMap, BinaryHeap, heap, Trie, memoization, DP, sliding window, two pointers, sort_unstable, select_nth_unstable, partition_point, binary_search, iterator, fold, zip, windows, chunks, 算法优化, 时间复杂度'
---
...
1. **Data Structure Upgrade**
   - Lookup/dedup: `Vec` (O(N)) → `HashSet`/`HashMap` (O(1) average)
   - Ordered lookup / range: linear scan → `BTreeMap`/`BTreeSet` (O(log N); `range()` is O(log N) + count)
   - Top-K: full sort (O(N log N)) → size-K `BinaryHeap` (O(N log K) scan)
...
| Append / insert | O(1) amortized at end | **O(1)** avg | O(log N) | O(log N) |
...
| Top-K from N items | O(N log N) | — | — | **O(N log K)** size-K heap; extraction adds O(K log K) |
...
| Stable (equal elements keep order) | `sort` / `sort_by` / `_by_key` | O(N log N), input-dependent auxiliary memory | Use when stability is required |
...
| Small arrays | Benchmark both variants | Implementation details vary by Rust version | Do not treat a tiny-input fast path as a general complexity rule |

> Prefer `sort_unstable_*` when stability is unnecessary and benchmark the workload. Use `select_nth_unstable` for selection when a full sort is not required.
...
- **Collect-then-reloop**: `.collect()` then iterate again when ownership or random access is unnecessary — chain combinators or collect once at the boundary.
...
| `.collect::<Vec<_>>()` then `.iter()` again without a boundary need | Chain; collect once at the boundary |
