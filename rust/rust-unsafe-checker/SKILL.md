...
Use `rust-resource-lifecycle` for ordinary `Drop`, lease, and cleanup design; use `rust-concurrency-testing` for deterministic interleavings and test isolation; use `rust-ecosystem` for native dependency and build portability review. Keep this skill focused on the soundness proof at the unsafe or FFI boundary.
