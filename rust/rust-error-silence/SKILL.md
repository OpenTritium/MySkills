...
3. **Avoid Dual Printing** — Logging AND propagating the same error (`error!(?e); return Err(e)`) often duplicates traces and alerts. Prefer logging at the handling boundary; intermediate layers add context and return unless they add distinct actionable information. (Full treatment: `rust-logging-review`.)
