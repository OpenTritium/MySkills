...
5. **Side Effect Declaration** — Make important I/O and mutation discoverable in the API or name. `get_user()` that updates a cache may surprise callers; consider `fetch_and_cache_user()` or a documented convention. (`rust-naming-smell` owns the rename; this rule owns whether the side effect belongs.)
...
| `fn get_user(id)` writing metrics / updating LRU | Split the side effect out, or rename `fetch_and_cache_user()` (`rust-naming-smell`) |
