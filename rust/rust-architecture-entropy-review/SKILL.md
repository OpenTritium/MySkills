---
name: rust-architecture-entropy-review
...
When a refactor or bug report leaves the behavior contract uncertain, require focused happy/unhappy characterization or regression tests through the stable boundary before endorsing a new owner or route. Use `rust-testing-strategy` for the test design; do not infer intended behavior from the new structure.
...
Use this skill when a local smell has architectural consequences. Defer purely local concerns to the narrower skill: function shape to rust-func-smell, module/type structure to rust-structure-refactor, type invariants to rust-encode-invariant, dependency integration to rust-ecosystem, and tests to rust-testing-strategy. Reuse their evidence when it proves that a route, owner, or boundary multiplied.
