...
3. **Define HTTP headers explicitly**
   - Require HTTPS and put credentials in `Authorization`, never in query parameters. Stripe authenticates server-side requests with an API key as the HTTP Basic Auth username; a new API should choose one documented auth scheme and keep secret keys server-side.
   - Use `Idempotency-Key` on retryable `POST` or other side-effecting writes only. Make keys high-entropy, non-sensitive, bounded in length, scoped, and retained for a documented period; replay the original status and body for the same parameters and reject parameter changes.
   - Pin request behavior with a version header such as `Stripe-Version`. If a platform acts for a connected account, carry that authorization context in an explicit header such as `Stripe-Account`; define precedence when the same context also appears in the URL or client configuration.
   - Return a stable `Request-Id` on every response and preserve it in logs and error envelopes. Send `Retry-After` when the client has a meaningful retry time. Declare request and response media types; Stripe's v1 API accepts form-encoded requests and returns JSON, while a JSON API should require `Content-Type: application/json` for JSON bodies.
   - For signed callbacks, put the signature in a header such as `Stripe-Signature` and verify it against the exact raw body before parsing. Treat the signature header, timestamp tolerance, replay protection, and secret rotation as one contract.

4. **Keep responses compact and composable**
...
5. **Make failures machine-actionable**
...
6. **Version behavior deliberately**
...
7. **Design webhooks as at-least-once delivery**
...
- Use `rust-state-machine` for internal legal states and transitions; use `rust-error-silence` or `rust-snafu` for Rust error implementation.
- Use `rust-async-concurrency` and `rust-concurrency-testing` for runtime task ownership, cancellation, synchronization, and forced interleavings behind the contract.
- Use `rust-testing-strategy` for broader test-level and fixture choices; use `rust-architecture-entropy-review` when multiple modules or routes create competing API sources of truth.
...
- [Connect authentication](https://docs.stripe.com/connect/authentication)
